...
BENCHMARK_INPUT_DIR = REPO_ROOT / "benchmark_inputs/communicator_vs_te"

COMM_CASES = {
    "map8_id": BENCHMARK_INPUT_DIR / "full8/map8_identity/result.json",
    "map8_within_swap": BENCHMARK_INPUT_DIR / "full8/map8_within_swap/result.json",
    "map8_half_swap": BENCHMARK_INPUT_DIR / "full8/map8_half_swap/result.json",
    "map8_target_bad_local": BENCHMARK_INPUT_DIR
    / "full8/map8_target_bad_local/result.json",
    "single_big_read": BENCHMARK_INPUT_DIR / "single_nic/comm_big_read.json",
    "single_nic_probe": BENCHMARK_INPUT_DIR / "single_nic/comm_probe.json",
}
...
SINGLE_NIC_CASES = {
    "ib_write_aligned": BENCHMARK_INPUT_DIR / "single_nic/ib_write.json",
    "ib_read_aligned": BENCHMARK_INPUT_DIR / "single_nic/ib_read.json",
    "te_write_aligned": BENCHMARK_INPUT_DIR / "single_nic/te_write.json",
    "te_read_aligned": BENCHMARK_INPUT_DIR / "single_nic/te_read.json",
}

LARGE_SINGLE_REQUEST_CASE = (
    BENCHMARK_INPUT_DIR / "single_nic/large_single_request_summary.json"
)
LARGE_SINGLE_32M_CASE = BENCHMARK_INPUT_DIR / "single_nic/large_single_32m_summary.json"
...
def communicator_case_metrics(path: Path) -> dict[str, float]:
    ...
    if lanes:
        ...
        case_wall_GBps = float(
            aggregate.get("bw_GBps_case_wall", aggregate.get("bw_gbps_case_wall", 0.0))
        )
        return {
...
def render_bar_chart(
    *,
    ...
    parts.append(
        f'<line x1="{left}" y1="{bottom}" x2="{left + chart_w}" y2="{bottom}" stroke="#222" stroke-width="2"/>'
    )
    ...
    for idx, (label, value, color) in enumerate(
        zip(labels, values, colors, strict=False)
    ):
        x = left + gap * idx + (gap - bar_width) / 2
        bar_h = 0 if max_value <= 0 else chart_h * value / max_value
        y = bottom - bar_h
        parts.append(
            f'<rect x="{x:.1f}" y="{y:.1f}" width="{bar_width:.1f}" height="{bar_h:.1f}" rx="4" fill="{color}"/>'
        )
        parts.append(
            svg_text(
                x + bar_width / 2,
    ...
    *,
    ...
    for cat_idx, category in enumerate(categories):
        ...
        for line_idx, segment in enumerate(category.split("\n")):
            parts.append(
                svg_text(
                    left + group_width * cat_idx + group_width / 2,
                    bottom + 24 + line_idx * 16,
                    segment,
                    size=12,
                    anchor="middle",
    ...
    for idx, (name, _, color) in enumerate(series):
        y = legend_y + idx * 24
        parts.append(
            f'<rect x="{legend_x}" y="{y - 12}" width="16" height="16" fill="{color}" rx="2"/>'
        )
    ...
    *,
    ...
    parts = [
        ...
        svg_text(
            width / 2,
            34,
            "Single-NIC Large-Block Read Sweep",
            size=30,
            weight="bold",
            anchor="middle",
        ),
        ...
        svg_text(
            width / 2,
            82,
            "Each size uses a documented source GPU/NIC to target GPU/NIC pair from the supplied benchmark input bundle.",
            size=12,
            anchor="middle",
        ),
        svg_text(
            chart_left,
            112,
            "Large Single-Request Read Throughput",
            size=18,
            weight="bold",
        ),
    ]
    ...
    for i in range(6):
        ...
        parts.append(
            svg_text(chart_left - 12, y + 5, f"{value:.1f}", size=12, anchor="end")
        )
    parts.append(
    ...
    legend_y = 118
    parts.append(
        f'<rect x="{legend_x}" y="{legend_y - 12}" width="16" height="16" rx="2" fill="#2a7f62"/>'
    )
    parts.append(svg_text(legend_x + 24, legend_y, "Communicator", size=12))
    parts.append(
        f'<rect x="{legend_x + 120}" y="{legend_y - 12}" width="16" height="16" rx="2" fill="#9c4f2f"/>'
    )
    ...
    for idx, category in enumerate(categories):
        group_x = (
            chart_left + group_width * idx + (group_width - total_group_bar_width) / 2
        )
        comm_val = comm_large[idx]
        te_val = te_large[idx]
        for series_idx, (value, color) in enumerate(
            ((comm_val, "#2a7f62"), (te_val, "#9c4f2f"))
        ):
        ...
        parts.append(
            svg_text(
                chart_left + group_width * idx + group_width / 2,
                chart_bottom + 26,
                category,
                size=13,
                anchor="middle",
...
def main() -> int:
    ...
    comm_metrics = {
        name: communicator_case_metrics(path) for name, path in COMM_CASES.items()
    }
    single_nic = {
        "ib_write_aligned_GBps": extract_ib_bw_gbps(
            SINGLE_NIC_CASES["ib_write_aligned"]
        )
        / 8.0,
        "ib_read_aligned_GBps": extract_ib_bw_gbps(SINGLE_NIC_CASES["ib_read_aligned"])
        / 8.0,
        "te_write_aligned_GBps": extract_mooncake_throughput_GBps(
            SINGLE_NIC_CASES["te_write_aligned"]
        ),
        "te_read_aligned_GBps": extract_mooncake_throughput_GBps(
            SINGLE_NIC_CASES["te_read_aligned"]
        ),
        "comm_best_known_GBps": comm_metrics["single_big_read"]["per_lane_GBps"],
        "comm_fresh_probe_GBps": comm_metrics["single_nic_probe"]["per_lane_GBps"],
        "aligned_pair": "node-a GPU2/nic2 -> node-b GPU2/nic2",
        "comm_best_known_case": str(COMM_CASES["single_big_read"]),
        "comm_fresh_probe_case": str(COMM_CASES["single_nic_probe"]),
    }
    large_single = load_large_single_request_data(LARGE_SINGLE_REQUEST_CASE)
    large_single_32m = load_json(LARGE_SINGLE_32M_CASE)
    large_single["communicator_GBps"]["32m"] = extract_summary_field(
        large_single_32m["communicator_summary"], "bw_GBps"
    )
    large_single["transfer_engine_GBps"]["32m"] = float(
        large_single_32m["transfer_engine_GBps"]
    )
    ...
    large_single["transfer_engine_GBps"]["64m"] = extract_mooncake_throughput_GBps(
        BENCHMARK_INPUT_DIR / "single_nic/te_big_read.json"
    )
        ...
        series=[
            (
                ...
                [
                    ...
                    MOONCAKE["map8_target_bad_local_gbps"]
                    / MOONCAKE["map8_identity_gbps"],
                ],
                "#7f5539",
            ),
            (
                ...
                [
                    ...
                    comm_metrics["map8_half_swap"]["data_plane_gbps"]
                    / comm_metrics["map8_id"]["data_plane_gbps"],
                    comm_metrics["map8_target_bad_local"]["data_plane_gbps"]
                    / comm_metrics["map8_id"]["data_plane_gbps"],
                ],
                "#2a7f62",
            ),

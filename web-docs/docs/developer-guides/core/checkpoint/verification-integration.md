...
description: Lightweight model integrity verification solution for storage and transfer
sidebar_position: 5
---

# Model Integrity Verification

This document summarizes the lightweight model integrity verification solution integrated in the model storage, loading, and transfer system.

## Design Philosophy

- **Zero-Copy Design**: Directly verify loaded model memory to avoid additional memory allocation
- **Chunked Streaming Computation**: Process large model data chunk by chunk using small buffers (1MB)
...
- Check key positions at the beginning, middle, and end of the model
...
- Divide model into 8 segments, calculate independent checksum for each segment
...
### 1. Model Saving Phase (`torch_util.py` + `checkpoint.cc`)
...
### 2. Model Loading Phase (`torch_util.py`)

```python
# In load_dict_non_blocking function
if enable_verification:
    verification_result = verify_model_data_from_gpu(
...
### 4. Model Class Interface (`model.h` + `model.cc`)

```cpp
// Generate verification information
absl::StatusOr<ModelVerificationInfo> generate_verification_info(ModelLocation location);

// Verify model data
absl::Status verify_model_data(ModelLocation location,
...
absl::Status verify_key_points(ModelLocation location,
                               const ModelVerificationInfo& expected_info);
```
...
### 5. Configuration Support (`model_config.h`)
...
save_dict(state_dict, "/path/to/model")
...
### C++-side Model Operations

```cpp
// Generate verification information
auto verification_info = model->generate_verification_info(ModelLocation::GPU);

// Fast verification
auto status = model->verify_key_points(ModelLocation::GPU, verification_info.value());

// Full verification
auto status = model->verify_model_data(ModelLocation::GPU, verification_info.value(),
...
auto verification_info = model->generate_verification_info(register_location);
...
- **Key Points Verification**: < 1ms (any model size)
- **Segment Verification**: ~10ms (67B parameter model)
- **Full Verification**: ~100ms (67B parameter model)
...
This integration solution provides complete model lifecycle verification coverage, providing reliable data integrity guarantees while maintaining extremely low overhead.
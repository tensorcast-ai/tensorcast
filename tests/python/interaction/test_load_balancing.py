...
def _register_replica(gs, model_name: str, node_id: str, max_concurrency: int, size_bytes: int = 1_000_000):
...
def test_load_balancing_concurrency(global_store_service):
    ...
    for node_id, cap in capacities.items():
        _register_replica(gs, model_name, node_id=node_id, max_concurrency=cap)
    ...
    for replica in gs.model_replica_repository.find_by_model(model_name):

...
class MockDistributedVirtualMemoryPool : public memory::DistributedVirtualMemoryPool {
 public:
  ...
  // === Overrides ===
  absl::StatusOr<VirtualRegion> allocate(std::string_view /*model_id*/, size_t bytes, int /*numa*/) override {
  ...
  absl::Status lock_chunks(std::string_view /*model_id*/, absl::Span<const uint32_t> idx) override {
    last_locked_chunks_.assign(idx.begin(), idx.end());
    return lock_status_;
  }

  absl::Status unlock_chunks(std::string_view /*model_id*/, absl::Span<const uint32_t> idx, bool copied_gpu) override {
    last_unlocked_chunks_.assign(idx.begin(), idx.end());
    last_unlock_mark_preemptible_ = copied_gpu;
    return unlock_status_;
  }

  absl::Status mark_preemptible(std::string_view /*model_id*/, absl::Span<const uint32_t> idx) override {
...
TEST_CASE("ModelMemoryCoordinator get missing chunks", "[unified_memory]") {
  ...
  SECTION("missing chunks for GPU") {
    ...
    REQUIRE(
        unified_memory.update_chunk_states(key, ModelLocation::GPU, {0, 2}, ChunkState::HOT, device_key.ordinal).ok());

    // Get missing chunks for GPU
    auto missing = unified_memory.get_missing_chunks(key, ModelLocation::GPU, device_key.ordinal);
  ...
  SECTION("missing chunks for CPU") {
    ...
    auto missing = unified_memory.get_missing_chunks(key, ModelLocation::PAGEABLE_CPU);
    REQUIRE(missing.size() == 1);
    REQUIRE(missing[0] == 1);
  }
}
...
TEST_CASE("ModelMemoryCoordinator lock chunks for transfer", "[unified_memory]") {
  ...
  SECTION("lock CPU chunks for GPU transfer") {
    ...
    REQUIRE(
        unified_memory.lock_chunks_for_transfer(key, ModelLocation::PAGEABLE_CPU, ModelLocation::GPU, chunks_to_lock)
            .ok());
...
TEST_CASE("ModelMemoryCoordinator update chunk states", "[unified_memory]") {
  ...
  SECTION("update GPU chunk states") {
    ...
    REQUIRE(
        unified_memory.update_chunk_states(key, ModelLocation::GPU, chunks, ChunkState::COPIED_GPU, device_id).ok());
  ...
  SECTION("update CPU chunk states") {
    ...
    REQUIRE(unified_memory.update_chunk_states(key, ModelLocation::PAGEABLE_CPU, chunks, ChunkState::COLD).ok());
...
TEST_CASE("ModelMemoryCoordinator mark CPU chunks preemptible", "[unified_memory]") {
  ...
  SECTION("mark 50% preemptible") {
    // Mock DVMP returning virtual memory for the model
...
TEST_CASE("ModelMemoryCoordinator chunk calculations", "[unified_memory]") {
  ...
  SECTION("various model sizes") {
    ...
    for (const auto& tc : test_cases) {
      DYNAMIC_SECTION("size " << tc.model_size) {
        InstanceKey key{absl::StrCat("model", tc.model_size), DeviceKey{DeviceType::GPU, 0, ""}, 0};
...
TEST_CASE("ModelMemoryCoordinator multi-GPU state tracking", "[unified_memory]") {
  ...
  REQUIRE(unified_memory.update_chunk_states(key, ModelLocation::GPU, {0}, ChunkState::HOT, gpu0).ok());
  REQUIRE(unified_memory.update_chunk_states(key, ModelLocation::GPU, {0, 1}, ChunkState::HOT, gpu1).ok());

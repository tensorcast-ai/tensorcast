...
TEST_CASE("DiskLoader streaming disk load to GPU", "[loader][disk][streaming][gpu]") {
  ...
  // Setup dummy model directory with two partitions
  const std::string model_subdir = "loader_streaming_files";
  ...
  // Setup DiskLoader
  DiskSource source;
  source.path = base / model_subdir;
  DiskLoader loader(source);
  REQUIRE(loader.initialize().ok());
  auto size_status = loader.get_model_size();
  REQUIRE(size_status.ok());
  const uint64_t model_size = *size_status;
  REQUIRE(model_size == total_size);
  ...
  REQUIRE(memmgr->get_state(ModelLocation::GPU) == MemoryState::LOADED);

  // Copy back to host and verify content
  auto gpu_ptrs = memmgr->get_pointer(ModelLocation::GPU);
  REQUIRE(gpu_ptrs.size() == 1);
  void* gpu_ptr = gpu_ptrs[0];
  REQUIRE(gpu_ptr != nullptr);

  std::vector<char> host_buf(model_size);
  absl::Status copy_status = stepcast::cuda::memcpy(host_buf.data(), gpu_ptr, model_size, cudaMemcpyDeviceToHost);

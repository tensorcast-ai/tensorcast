...
"""
Global configuration management for scstore.

This module handles the centralized configuration and initialization
of the scstore system, including daemon management.
"""
...
def init(
    ...
    """
    Initialize scstore client and optionally manage daemon lifecycle.

    This function serves as the central configuration point for scstore. It can:
    ...
    Examples:
        ...
        scstore.init(storage_path="./test-models", daemon_mem_pool_size="64GB")

        # Connect to existing daemon only
        scstore.init(connect_only=True)

        # Configure but don't start daemon yet
        scstore.init(auto_start=False, storage_path="./models")
    """

    # Import here to avoid circular imports
    from scstore.daemon_manager import ensure_daemon_running, get_daemon_manager
    from scstore.logger import init_logger
    from scstore.store_daemon.config import ServerConfig, StoreDaemonConfig
    from scstore.torch_util import set_daemon_address
    from scstore.utils import to_num_bytes

    logger = init_logger(__name__)

    if os.environ.get("SCSTORE_FORCE_CONNECT_ONLY", "false").lower() in [
        "true",
        "1",
        "yes",
        "y",
    ]:
        logger.info("SCSTORE_FORCE_CONNECT_ONLY is set, forcing connect_only=True")
        connect_only = True
        auto_start = False
    ...
    if connect_only:
        ...
        logger.info("Initializing scstore with auto daemon management...")
        ...
        if success:
            set_daemon_address(daemon_address)
            logger.info("✅ scstore initialized successfully")
        else:
            logger.error("❌ Failed to initialize scstore")
            raise RuntimeError("Failed to initialize scstore")

    else:
        # Just set configuration, don't start daemon
        logger.info("scstore configured (daemon not started)")
        set_daemon_address(daemon_address)

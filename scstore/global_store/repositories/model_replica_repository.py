...
"""Repository for model replica data access."""
...
class ModelReplicaRepository(BaseRepository):
    ...
    def create(self, replica: ModelReplica) -> ModelReplica:
        """Create a new replica."""
        cursor = self.get_cursor()

        # Insert into model_replicas (without current_requests)
        cursor.execute(
            """
    ...
    def create_or_update_atomic(self, replica: ModelReplica, cursor) -> ModelReplica:
        """
        ...
        Args:
            replica: ModelReplica to create or update
            cursor: Database cursor within an active transaction

        Returns:
            Created or updated ModelReplica
        """
    ...
    def update(self, replica: ModelReplica) -> ModelReplica:
    ...
    def decrement_requests(self, replica_id: UUID) -> tuple[int, int]:
        ...
        # Fetch max_concurrency from model_replicas
        max_row = cursor.execute(
            """
            SELECT max_concurrency FROM model_replicas WHERE replica_id = ?
            """,
            [str(replica_id)],
        ).fetchone()
    ...
    def mark_unavailable_by_worker(self, worker_id: str) -> int:
        ...
        count_result = cursor.execute(
            """
            SELECT COUNT(*) FROM model_replicas
    ...
    def list_all_replicas(self) -> list[ModelReplica]:
    ...
    def find_orphaned_replicas(self) -> list[ModelReplica]:
    ...
    def get_replicas_by_worker(self, worker_id: str) -> list[ModelReplica]:
    ...
    def get_stale_replicas(self, recovery_time: int) -> list[ModelReplica]:

...
namespace tensorcast::store::materialization::planning {
...
absl::Status ChunkAwareLoadingStrategy::execute_p2p_transfer(
  ...
  for (const auto& [source_key, chunks] : chunks_by_source) {
    ...
    auto loader = std::make_unique<DiskLoader>(
        store::loading::DiskSource{.path = std::filesystem::path{}, .expected_size = std::nullopt});

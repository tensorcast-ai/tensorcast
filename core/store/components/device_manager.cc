...
DeviceManager::~DeviceManager() {
  ...
  for (auto& [device_id, gpu_info] : gpu_info_map_) {
    if (gpu_info.stream != nullptr) {
      auto status = stepcast::cuda::set_device(device_id);
      if (!status.ok()) {
        LOG(ERROR) << "Failed to set device " << device_id << " during cleanup: " << status.message();
      }
      status = stepcast::cuda::stream_destroy(gpu_info.stream);
      if (!status.ok()) {
...
absl::Status DeviceManager::initialize() {
  auto count_status = stepcast::cuda::get_device_count(&num_gpus_);
  if (!count_status.ok()) {
    return count_status;
  }
  ...
  for (int i = 0; i < num_gpus_; ++i) {
    auto device_status = stepcast::cuda::set_device(i);
    if (!device_status.ok()) {
      return device_status;
    }

    cudaDeviceProp props;
    auto props_status = stepcast::cuda::get_device_properties(i, &props);
    if (!props_status.ok()) {
      return props_status;
    }
    ...
    // Create CUDA stream
    auto stream_status = stepcast::cuda::stream_create(&info.stream);
    if (!stream_status.ok()) {
    ...
    auto mem_info_status = stepcast::cuda::get_memory_info(&free_mem, &total_mem, i);
    ...
    gpu_memory_total_gauges_.emplace(
        i,
        stepcast::metrics::Gauge(
            "store_daemon_gpu_memory_bytes", {{"device_id", device_id_str}, {"memory_type", "total"}}));
    gpu_memory_free_gauges_.emplace(
        i,
        stepcast::metrics::Gauge(
            "store_daemon_gpu_memory_bytes", {{"device_id", device_id_str}, {"memory_type", "free"}}));
...
void DeviceManager::update_gpu_metrics() {
  for (const auto& [device_id, info] : gpu_info_map_) {
    ...
    size_t free_mem, total_mem;
    auto mem_status = stepcast::cuda::get_memory_info(&free_mem, &total_mem, device_id);
    if (mem_status.ok()) {
...
absl::StatusOr<size_t> DeviceManager::get_free_memory(int device_id) {
  ...
  size_t free_mem, total_mem;
  auto final_mem_status = stepcast::cuda::get_memory_info(&free_mem, &total_mem, device_id);

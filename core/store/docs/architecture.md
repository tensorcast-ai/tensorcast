...
| RuntimeContext | `core/store/runtime/context/runtime_context.{h,cc}` | Constructs `DeviceManager`, `ReplicaRegistry`, `MetricsCollector`, `CommunicationManager`, `PinnedBufferPool`, `ViewHashComputer`, `StableDramCacheManager`, and either initializes or reuses an injected `IGlobalStoreClient`; propagates worker identity and embeds the event dispatcher. | `StoreEngineOptions`, `WorkerIdentity` |
...
| StableDramCacheManager | `core/store/components/stable_dram_cache_manager.{h,cc}` | Tracks local stable DRAM retention (best-effort/TTL/pinned), admits entries by acquiring UMA stable leases with de-duplicated accounting, filters eviction candidates, enforces shared-disk spill guards with durability-gated spill eviction, clears tracking after stable lease release, and emits cache metrics. | `ReplicaRegistry`, `MemoryTierBudget`, UMA |

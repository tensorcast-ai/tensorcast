...
- Service helpers:
  ...
  - Runtime catalog + services (core/store/runtime/**):
    ...
    - IngestionRuntime: core/store/runtime/ingestion/ingestion_runtime.{h,cc} (orchestrates ingestion pipelines and delegates UMA allocations to ReplicaRuntime; exposes `IngestionRuntimeDependencies` so tests can inject fake pipelines, coordinators, or event sinks without touching production wiring).
...
`StoreEngine` is treated as a thin runtime facade. Construction delegates to `runtime::RuntimeEnv`, which validates options, initializes DeviceManager/ReplicaRegistry/MetricsCollector, wires the shared `PinnedBufferPool`, owns worker identity, and optionally connects to Global Store. Replica lifecycle helpers (`get_resident_devices`, `wait_replica_ready`, UMA snapshots, remote access toggles, eviction retries, etc.) are provided by `runtime::ReplicaRuntime`, so `StoreEngine` no longer reaches directly into `ReplicaRegistry`/`DeviceManager`. `runtime::metadata::MetadataGateway` is the sole component that talks to Global Store; `StoreEngine`, the ingestion pipeline, and the registration backend emit events and the gateway translates them into register/unregister/key-mapping RPCs. `runtime::IngestionRuntime` unifies ingestion (disk, P2P, inline) flows while emitting structured events through the RuntimeContext dispatcher, keeping orchestration focused on intent while dedicated modules encapsulate lifecycle, telemetry, and metadata ownership. ReplicaRuntime and MetadataGateway now receive ingestion completion payloads directly (in addition to the RuntimeContext events), so UMA telemetry updates and Global Store registration happen automatically without wiring bespoke event handlers into the dispatcher. Memory-registration commits and aborts still emit RuntimeContext payloads for observers, but MetadataGateway also refreshes UMA metrics inline so StoreEngine never needs to thread callbacks through the façade.

...
- `StableDramCacheManager` gates local stable-DRAM retention by acquiring UMA stable leases on admission, applies per-entry retention policy (`best_effort`, `ttl`, `pinned`), filters eviction candidates during demand-driven cache pressure, and treats `overflow_policy=spill` as a hard requirement on shared-disk availability plus a spill-evictable durability check before evicting.

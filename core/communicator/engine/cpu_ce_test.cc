...
int run_server() {
  ...
  CHECK_OK(engine.init(g_ip, g_port));
  ...
  opts.async = false;
  CHECK_OK(engine.register_tensor_ex("cpu-ce-test-tensor", reinterpret_cast<uint64_t>(addr), g_count, 0, 1, opts));
  while (true) {
...
int run_client() {
  ...
  printf(
      ...
      "read=%lu, cost=%lu\n",
      static_cast<int>(ret.status.code()),
      ret.request_cost,
      ret.rdma_queue_cost,
      ret.read_cost,

...
- **Topology is immutable; bindings are refreshable in place**: `set_topology` remains one-shot, `set_endpoint_bindings` remains one-shot initialization, and `upsert_endpoint_bindings` / `update_endpoint_binding` refresh endpoint bindings on the live `RoutingContext` while bumping topology generation so cached channels rebuild against the new bindings.

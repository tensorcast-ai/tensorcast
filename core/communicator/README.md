...
- **Cross-node rail-matched fallback** is now supported for direct-channel construction: when no direct topology link exists and source/destination bindings are on different nodes, routing selects a local NIC by GPU/pool/rail affinity, then selects a remote binding on the destination node by rail-aware scoring (preferred rail, destination rail, endpoint-id/NIC heuristics, network address). This allows runtime endpoint ids (for example `node/dev/gpu/<id>`) to route through NIC bindings even when those runtime ids are not topology endpoint ids.

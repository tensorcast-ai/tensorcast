...
communicator:
  ...
  const std::string path = write_temp_file(yaml, "legacy_prewarm_alias.yaml");
  auto cfg_or = LoadCommunicatorConfigFromFile(path);
  REQUIRE(cfg_or.ok());
  const CommunicatorConfig& cfg = cfg_or.value();

  REQUIRE(cfg.rdma().stable_local_mr_reuse_eager_prereg_all_rails() == true);
  REQUIRE(cfg.rdma().has_stable_local_mr_reuse_prewarm_workers() == true);
  REQUIRE(cfg.rdma().stable_local_mr_reuse_prewarm_workers() == 1);
}

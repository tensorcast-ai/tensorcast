...
struct ibv_qp_init_attr {
  ...
  int sq_sig_all;
  /* Below is needed for backwards compatible */
...
struct ibv_srq {
  ...
  /* below are for source compatibility with legacy XRC,
...
struct ibv_context_ops {
  ...
  struct ibv_mr* (*rereg_mr)(struct ibv_mr* mr, int flags, struct ibv_pd* pd, void* addr, size_t length, int access);
...
struct verbs_context {
  ...
  struct ibv_qp* (*create_qp_ex)(struct ibv_context* context, struct ibv_qp_init_attr_ex* qp_init_attr_ex);
  int (*get_srq_num)(struct ibv_srq* srq, uint32_t* srq_num);
  struct ibv_srq* (*create_srq_ex)(struct ibv_context* context, struct ibv_srq_init_attr_ex* srq_init_attr_ex);
  struct ibv_xrcd* (*open_xrcd)(struct ibv_context* context, struct ibv_xrcd_init_attr* xrcd_init_attr);

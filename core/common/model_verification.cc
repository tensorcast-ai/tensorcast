...
std::string ModelVerificationInfo::to_json() const {
  json j;
  j["version"] = "1.0";
...
absl::StatusOr<ModelVerificationInfo> ModelVerificationInfo::from_json(const std::string& json_str) {
  try {
    ...
    // Parse model_size
    if (j.contains("model_size") && j["model_size"].is_number_unsigned()) {
      info.model_size = j["model_size"].get<uint64_t>();
    }
...
uint64_t ModelVerifier::xxhash64(const void* data, size_t len, uint64_t seed) {
...
absl::Status ModelVerifier::read_data_chunk(
    void* dest,
...
absl::StatusOr<uint64_t> ModelVerifier::get_value_at_offset(
    const std::vector<void*>& data_ptrs,
    const std::vector<size_t>& data_sizes,
    size_t offset,
    int device_id) {
...
absl::StatusOr<ModelVerificationInfo> ModelVerifier::generate_verification_info(
    const std::vector<void*>& data_ptrs,
    const std::vector<size_t>& data_sizes,
    int device_id,
  ...
  ModelVerificationInfo info;
  info.model_size = std::accumulate(data_sizes.begin(), data_sizes.end(), 0UL);

  if (info.model_size == 0) {
    return absl::InvalidArgumentError("Model size is 0");
  }
  ...
  auto mid_val = get_value_at_offset(data_ptrs, data_sizes, info.model_size / 2, device_id);
  if (!mid_val.ok()) {
    return mid_val.status();
  }
  info.key_values[1] = *mid_val;

  auto last_val = get_value_at_offset(data_ptrs, data_sizes, info.model_size >= 8 ? info.model_size - 8 : 0, device_id);
  if (!last_val.ok()) {
  ...
  for (size_t i = 0; i < NUM_SAMPLES; ++i) {
    size_t offset = (info.model_size * i) / NUM_SAMPLES;
  ...
  if (max_level >= VerificationLevel::SEGMENT_HASHES) {
    ...
    size_t segment_size = info.model_size / NUM_SEGMENTS;
    uint64_t rolling_hash = 0;

    for (size_t seg = 0; seg < NUM_SEGMENTS; ++seg) {
      size_t seg_start = seg * segment_size;
      size_t seg_end = (seg == NUM_SEGMENTS - 1) ? info.model_size : (seg + 1) * segment_size;
  ...
  LOG(INFO) << "Generated model verification info for " << info.model_size << " bytes in " << duration.count() << " ms"
            << (device_id >= 0 ? " (GPU)" : " (CPU)");

  return info;
}

absl::Status ModelVerifier::verify_key_points(
    const std::vector<void*>& data_ptrs,
    const std::vector<size_t>& data_sizes,
    const ModelVerificationInfo& expected_info,
    int device_id) {
  uint64_t total_size = std::accumulate(data_sizes.begin(), data_sizes.end(), 0UL);
  if (total_size != expected_info.model_size) {
    return absl::FailedPreconditionError(
        absl::StrFormat("Size mismatch: got %lu, expected %lu", total_size, expected_info.model_size));
  }
  ...
  if (*first_val != expected_info.key_values[0]) {
    return absl::DataLossError(
        absl::StrFormat(
            "Key point mismatch at start of model: got 0x%lx, expected 0x%lx",
            *first_val,
  ...
  if (*mid_val != expected_info.key_values[1]) {
    return absl::DataLossError(
        absl::StrFormat(
            "Key point mismatch at middle of model: got 0x%lx, expected 0x%lx", *mid_val, expected_info.key_values[1]));
  }
  ...
  if (*last_val != expected_info.key_values[2]) {
    return absl::DataLossError(
        absl::StrFormat(
            "Key point mismatch at end of model: got 0x%lx, expected 0x%lx", *last_val, expected_info.key_values[2]));
  }
...
absl::Status ModelVerifier::verify_model_data(
    const std::vector<void*>& data_ptrs,
    const std::vector<size_t>& data_sizes,
    const ModelVerificationInfo& expected_info,
  ...
  if (level >= VerificationLevel::SPARSE_SAMPLING) {
    for (size_t i = 0; i < NUM_SAMPLES; ++i) {
      size_t offset = (expected_info.model_size * i) / NUM_SAMPLES;
      auto val = get_value_at_offset(data_ptrs, data_sizes, offset, device_id);
      if (!val.ok()) {
        return val.status();
      }
  ...
  if (level >= VerificationLevel::SEGMENT_HASHES) {
    ...
    size_t segment_size = expected_info.model_size / NUM_SEGMENTS;

    for (size_t seg = 0; seg < NUM_SEGMENTS; ++seg) {
      size_t seg_start = seg * segment_size;
      size_t seg_end = (seg == NUM_SEGMENTS - 1) ? expected_info.model_size : (seg + 1) * segment_size;
  ...
  if (level >= VerificationLevel::FULL_HASH) {
    ...
    if (actual_info->full_hash != expected_info.full_hash) {
      return absl::DataLossError("Full model hash mismatch");
    }
  }
  ...
  VLOG(1) << "Model verification passed at level " << static_cast<int>(level) << " in " << duration.count() << " ms";

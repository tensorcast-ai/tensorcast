...
sequenceDiagram
  ...
  D->>IO: stream bytes
  D-->>SDK: success plus target_publication_token
  opt publish requested
    SDK->>D: publish target replica using target_publication_token
...
## Publish Requires A Daemon-Minted target_publication_token

To avoid re-hashing client VRAM on publish:

- the daemon mints a `target_publication_token` after a successful target materialization

...
- The host callback is attached via `cudaLaunchHostFunc` and is intentionally lightweight: return SPB slots, notify UMA/VS chunk completion, enqueue follow-on work. Runtime errors surface through `CopyHandle::wait()`; submission-time errors surface via `StatusOr`.

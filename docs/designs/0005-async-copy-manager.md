...
- ACM also supports inline completion hooks that run inside the host callback before a `CopyHandle` signals done; keep them non-blocking and free of CUDA API calls when used for per-copy bookkeeping that must be visible immediately after `CopyHandle::wait()`.

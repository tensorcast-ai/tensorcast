...
## Phase 2 – Registration & Put Consolidation
- Extract `_register._register_artifact_core` into pure helpers consumed by `Store.register`/`register_async`, returning the new `RegisteredArtifact` dataclass and assembling `ArtifactFuture` instances.
- Reuse the existing coalesced VRAM staging logic in `_register.py:639` within `Store.put` and `put_async`, ensuring AVBS hashing and canonical-index population remain untouched.
- Normalize error handling to raise `ArtifactError` with retry metadata, translating daemon status codes surfaced via `tensorcast.api._errors._wrap_status`.
- Emit per-endpoint metrics (`store.register.latency_ms`, `store.register.errors_total`) using the existing SDK metrics sink while tagging with `daemon_endpoint`.

## Phase 3 – Retrieval & In-Place Materialization
- Refactor `_loader.get_artifact_sync/async` into reusable functions that `Store.get`/`get_async` call, centralizing canonical index lookup and replica selection policy.
- Implement `Store.get_into`/`get_into_async` by reusing `_loader._validate_tensor_layout` and the CUDA IPC export path, guaranteeing PAD segments zero-fill before in-place writes.
...
- Validate that Global Store cache TTLs align with invalidation events to avoid stale key→artifact-id lookups; add metrics for cache hit/miss ratio.
- Engage SDK packaging to ensure the new module exports do not break wheel builds or Bazel `py_library` deps; update `setup.cfg` if re-export paths change.
- Keep release notes and migration guides in sync with `docs/plans/0014-store-session-api-modernization.md`; update status once each phase completes.

# References

- Design 0003 — Unified Memory Artifact Registration (AVBS + Coalesced VRAM + LIP).
- Design 0005 — Async Copy Manager (executor reuse for futures).
- Design 0006 — Unified Memory Stager and Staged P2P Transfers.

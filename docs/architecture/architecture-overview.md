...
- **Registration Path**: The RFC-0006 lifecycle (Begin → TTL → Commit/Abort) lives in `components::RegistrationFacade`, which maintains pending state, view ingestion, hashes, and emits registration events. `runtime::GlobalMetadataGateway` consumes those events (and the ingestion pipeline’s load events) to publish replicas and variant metadata to Global Store without routing through `StoreEngine`.
- **Runtime Modules**: `RuntimeEnv` owns lifecycle wiring and worker identity; `ReplicaRuntime` encapsulates replica lifecycle APIs; `ArtifactIngressManager` unifies ingestion + registration flows; `GlobalMetadataGateway` is the only component touching the Global Store client; and `RuntimeEventHub` provides synchronous fan-out for replica/materialization events. `StoreEngine` now delegates to these modules instead of manually orchestrating ComponentCatalog/Telemetry/Publisher wiring.

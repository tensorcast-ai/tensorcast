...
| Disk-first fallback on the client | `FallbackOptions(prefer="disk")` | Disk paths are resolved by the daemon from managed disk locations; the SDK only forwards `SourcePolicy(preference=PREFER_DISK, allow flags)` and the daemon reports the actual materialization source | [`MaterializationPipeline._materialize`](../../tensorcast/api/store/materialization.py) resolves key mappings, sets `preference`/allow flags, and `MaterializationPayload.source` mirrors the daemon-reported path. |

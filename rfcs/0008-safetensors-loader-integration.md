...
- Add header parser and dtype mapping in `tensorcast/api` (see `_indices.py`)
...
| `tensorcast/api/_indices.py` | Update | Parse header; build meta/data indices; identity offsets |
...
| 5 | Python header parse & merge + load paths | ✅ Implemented | `tensorcast/api/_indices.py` builds indices from `.safetensors` when no `tensor_index.json` |
...
- Python path: `tensorcast/api/_indices.py` now parses safetensors headers to produce unified meta/data indices when `tensor_index.json` is absent. Dtype mapping covers F16/BF16/F32/F64 and integer/byte/bool types.

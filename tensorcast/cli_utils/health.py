...
def wait_for_daemon(
    address: str, *, timeout: float = 10.0, interval: float = 0.2
) -> bool:
    ...
    while time.time() < deadline:
        if ping_daemon(address, timeout=min(wait_interval, timeout)):
            return True
...
def wait_for_global_store(
    ...
    wait_interval = max(0.05, interval)
    while time.time() < deadline:
        health = ping_global_store(address, timeout=min(wait_interval, timeout))

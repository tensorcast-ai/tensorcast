...
def _check_existing_daemon(pid_file: Path) -> None:
    ...
    if existing_pid and is_process_running(existing_pid):
        ...
        click.echo(f"Use 'tensorcast-cli stop' to stop it or remove {pid_file}")
...
def _ensure_cpp_daemon_binary() -> Path:
    ...
    Resolution order:
    - TENSORCAST_DAEMON_BIN env var (absolute path)
    - Installed package resource: tensorcast/bin/tensorcast_daemon
    - Development workspace: <repo-root>/bazel-bin/daemon/tensorcast_daemon
    """
    ...
    # 2) Packaged binary under tensorcast/bin/
    try:
    ...
    raise ServiceError(
        "tensorcast_daemon binary not found. Set TENSORCAST_DAEMON_BIN, install a package containing tensorcast/bin/tensorcast_daemon, or build it via Bazel in development (bazel build //daemon:tensorcast_daemon)."
    )

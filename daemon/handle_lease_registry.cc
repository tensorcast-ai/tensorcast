  ...
  if (init_status.ok()) {
    ...
    if (!stable_or.ok()) {
      ...
      if (!export_or.ok()) {
        ...
        ABSL_LOG_IF(WARNING, !rel.ok()) << "release_replica_stable_lease failed after set_exported failure: " << rel;
        stable_lease.reset();
...

  {
    ...
    if (it == cpu_exports_.end() || it->second.get() != state.get()) {
      ...
      if (stable_lease.has_value()) {
        auto rel = engine_->release_replica_stable_lease(*stable_lease);
        ABSL_LOG_IF(WARNING, !rel.ok()) << "release_replica_stable_lease failed: " << rel;
      }
...
void HandleLeaseRegistry::release_cpu_export_state_(
  ...
  if (!chunks.empty()) {
    ...
    ABSL_LOG_IF(WARNING, !st.ok()) << "set_replica_exported(off) failed: " << st.status();
  }
  if (stable_lease.has_value()) {
    auto st = engine_->release_replica_stable_lease(*stable_lease);
    ABSL_LOG_IF(WARNING, !st.ok()) << "release_replica_stable_lease failed: " << st;
  }
  if (fd_to_close >= 0) {
    ::close(fd_to_close);
  }

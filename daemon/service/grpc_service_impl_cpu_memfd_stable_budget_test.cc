...
TEST_CASE("CPU memfd stable_bytes recovers after lease release", "[daemon][cpu_memfd][stable_budget]") {
  ...
  const auto artifact_dir_a = root / "artifact_a";
  const auto artifact_dir_b = root / "artifact_b";
  ...
  auto materialize_cpu = [&](const std::filesystem::path& artifact_dir, const std::string& replica_uuid) {
    tensorcast::daemon::v2::MaterializeReplicaRequest req;
    req.set_disk_path(artifact_dir.string());
    req.set_target_device_type(tensorcast::daemon::v2::DeviceType::DEVICE_TYPE_CPU);
    req.set_preference(tensorcast::daemon::v2::SourcePreference::SOURCE_PREFERENCE_PREFER_DISK);
  ...
  auto [st_a, resp_a] = materialize_cpu(artifact_dir_a, "cpu_memfd_budget_a");
  REQUIRE(st_a.ok());
  ...
  auto [st_b, resp_b] = materialize_cpu(artifact_dir_b, "cpu_memfd_budget_b");
  ...
  auto [st_b2, resp_b2] = materialize_cpu(artifact_dir_b, "cpu_memfd_budget_b2");
  REQUIRE(st_b2.ok());

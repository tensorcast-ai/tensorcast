...
absl::StatusOr<std::string> maybe_mint_target_write_token(
  ...
  scope.mutable_byte_space()->CopyFrom(byte_space_from_selection(current_selection));
  ...
  auto token_or = capability_tokens->mint(
      identity.daemon_id(), tensorcast::common::v1::CAPABILITY_AUDIENCE_TARGET_WRITE, *scope_or, expires_at_ms);
  if (!token_or.ok()) {
  ...
  record.byte_space = byte_space_from_selection(current_selection);
  ...
  target_materialization_service->remember_target_write(std::move(record));
  return *token_or;
}
...
grpc::Status OwnedBindingService::create_owned_binding(
  ...
  auto target_write_token_or = maybe_mint_target_write_token(
  ...
  if (target_write_token_or.ok() && !target_write_token_or->empty()) {

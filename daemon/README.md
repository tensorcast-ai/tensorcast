...
- When `server.storage_path` is set, the daemon canonicalizes this root and applies it only to relative `disk_path` values; absolute `disk_path` values are accepted as-is. When `server.storage_path` is empty, disk materialization accepts absolute `disk_path` values only.

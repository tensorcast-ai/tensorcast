...
- Trusted mounted-source resolve now runs through typed `public_disk_source` daemon config. By default, the daemon derives one trusted root policy from `server.storage_path` and rejects absolute mounted-source paths outside that root. If `server.storage_path` is empty, operators must configure `public_disk_source.trusted_root_policies` explicitly before resolving mounted-source paths.
...
- Controllers:
  ...
  - StatusController: `GetServerConfig`, `GetWorkerStatus`, daemon-served directory RPCs
    ...
    `GetDetailedStatus`, `GetLoadedReplicas`.
- State/Kernel:
...
- Status: `GetServerConfig`, `GetWorkerStatus`, `GetDetailedStatus`, `GetLoadedReplicas` (paginated). `GetWorkerStatus`
...
- When a non-identity view is requested, `MaterializeReplicaResponse` populates `view_index_bytes` (canonicalised layout for the requested ByteSpace) and `view_data_hash` when verification completes. For canonical disk-source loads, `view_index_bytes` is also filled from the artifact directory so clients can reconstruct tensors without querying Global Store.

...
TC_OUT_DIR="${TC_OUT_DIR:-/tmp/tensorcast/cross_host/results_chaos}"
...
TC_ORCHESTRATOR_CHARGED_GROUP="${TC_ORCHESTRATOR_CHARGED_GROUP:-}"
...
if [[ "${TC_ORCHESTRATOR_ENABLE_LAUNCH}" == "true" ]]; then
  ORCHESTRATOR_BASE_CMD="orchestratorctl launch --gpu ${TC_ORCHESTRATOR_GPU} --cpu ${TC_ORCHESTRATOR_CPU} --memory ${TC_ORCHESTRATOR_MEMORY} --max-wait-duration=${TC_ORCHESTRATOR_MAX_WAIT_DURATION}"
  if [[ -n "${TC_ORCHESTRATOR_CHARGED_GROUP}" ]]; then
    ORCHESTRATOR_BASE_CMD+=" --charged-group=${TC_ORCHESTRATOR_CHARGED_GROUP}"
  fi

...
- Without GPUs, build and run with the Fake CUDA backend (see AGENTS.md). Fake CUDA simulates cross-process CUDA IPC (shared-memory backed handles) and supports the CUDA VMM wrapper calls used by internal benchmarks.
